...
def main() -> None:
    ...
    pred_train = predict_in_batches(model, z, PRED_BATCH_TRAIN)
    pred_test = predict_in_batches(model, z_test, PRED_BATCH_TEST)

    np.savetxt("pred_train_unet_64x64_4.txt", pred_train.reshape((-1, 4096)))
    np.savetxt("pred_test_unet_64x64_4.txt", pred_test.reshape((-1, 4096)))
